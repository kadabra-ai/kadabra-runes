...
async fn setup_client() -> LspClient {
    // Build the client with a longer timeout for initialization
    let client = LspClient::builder()
        .server_command(find_rust_analyzer())
        .workspace_root(fixture_path())
        .init_timeout(Duration::from_secs(60)) // Allow plenty of time for indexing
        .request_timeout(Duration::from_secs(30))
        .build()
        .await
        .expect("Failed to start LSP client");

    // Give rust-analyzer a moment to fully index the workspace
...
async fn open_file(client: &LspClient, relative_path: &str) -> PathBuf {
    ...
    // Give rust-analyzer a moment to process the file
